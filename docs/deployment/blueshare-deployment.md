...
> **Historical proposal:** The `src/core`, `src/platform`, CLI, and test paths
> below describe a planned service layout and are not commands for the cleaned
> repository. Current C demonstrations and public headers live under
> `native/blueshare-core/`. See `../current-status.md` before using this guide.
